...
CHAT_SYSTEM_PROMPT = (
    ...
    "   - 如果用户问的是通用建议、思路、方法，可以回答，但必须在开头标注 '💡'。\n"
...
CONTEXTUAL_SUGGEST_SYSTEM_PROMPT = (
    "你是 SageMate 的写作辅助助手，正在用户编辑 Wiki 时提供低打扰的关联知识提示。\n"
    "只基于提供的 Wiki 上下文和用户正在写的片段回答；不要引入外部知识。\n"
    "语气像自然插话，简短、克制、有帮助。\n"
    "不要写标题，不要写清单，不要编号，不要说“以下是”。\n"
    "如果确实有相关页面，请自然嵌入 [[slug]] 形式的 Wiki 链接；如果相关性弱，就回答空字符串。"
)


CONTEXTUAL_SUGGEST_PROMPT_TEMPLATE = """用户正在编辑 Wiki 页面。

当前页面：{page_title} ({page_slug})
光标附近片段：
{cursor_context}

已存在的 Wiki 链接：{existing_links}

检索到的相关页面：
{context}

请给出一句自然的写作辅助建议，帮助用户把当前片段和已有知识串起来。
要求：
- 最多 2 句话，总长度不超过 120 个中文字符
- 必须自然、克制，像旁注，不要打断写作
- 只在确有帮助时推荐；无明显关联时输出空字符串
- 如果引用页面，使用 [[slug]]，不要使用编号引用"""
...
class AgentPipeline:
    ...
    async def _process_with_intent_stream(self, msg: AgentMessage, bypass_router: bool = False):
        ...
        if msg.raw_data.get("intent") == "contextual_suggest":
            async for event in self._handle_contextual_suggest_stream(msg):
                yield event
            return
    ...
    @staticmethod
    def _stream_chunk_text(chunk) -> tuple[str, bool]:
        """Normalize LLM stream chunks across providers."""
        if hasattr(chunk, "text"):
            return chunk.text, bool(getattr(chunk, "is_reasoning", False))
        return str(chunk), False

    async def _handle_contextual_suggest_stream(self, msg: AgentMessage):
        """Stream low-interruption writing suggestions for the active editor."""
        raw = msg.raw_data or {}
        page_slug = str(raw.get("page_slug") or "").strip()
        page_title = str(raw.get("page_title") or page_slug or "当前页面").strip()
        cursor_context = str(raw.get("cursor_context") or msg.text or "").strip()
        full_content = str(raw.get("full_content") or "")
        existing_links = raw.get("existing_links") or re.findall(r"\[\[([^\]]+)\]\]", full_content)
        recent_suggestion_slugs = raw.get("recent_suggestion_slugs") or raw.get("exclude_slugs") or []

        if len(cursor_context) < 40:
            yield {"type": "done", "answer": "", "action_taken": "queried", "related_pages": []}
            return
        ...
        query = f"{page_title}\n{cursor_context[-1200:]}"
        try:
            results = await self.store.search(query, limit=8)
        except Exception as e:
            # Writing suggestions are opportunistic; never let them interrupt chat streaming.
            logger.warning(f"[ContextualSuggest] search failed: {e}")
            yield {"type": "done", "answer": "", "action_taken": "queried", "related_pages": []}
            return
        existing_link_set = {str(link) for link in existing_links if link}
        excluded_slugs = existing_link_set | {str(slug) for slug in recent_suggestion_slugs if slug}
        related_pages = []
        page_contents = []

        for r in results:
            if r.slug == page_slug or r.slug in excluded_slugs:
                continue
            page = await self.store.get_page(r.slug)
            if not page:
                continue
            ...
            page_contents.append(f"### {page.title} (slug: {page.slug})\n\n{content or page.summary or r.snippet or ''}")
            related_pages.append({
                "slug": page.slug,
                "title": page.title,
                "category": page.category.value if page.category else "concept",
        ...
        if not self.settings.llm_api_key:
            first = related_pages[0]
            answer = f"这里可能能和 [[{first['slug']}]] 串起来，后续可补一句它与“{page_title}”的关系。"
            yield {
                "type": "done",
                "answer": answer,
                "action_taken": "queried",
        ...
        try:
            from ...ingest.compiler.compiler import LLMClient
            llm = LLMClient(purpose="query")
            prompt = CONTEXTUAL_SUGGEST_PROMPT_TEMPLATE.format(
                page_title=page_title,
                page_slug=page_slug or "-",
                cursor_context=cursor_context[-1600:],
                existing_links=", ".join(existing_link_set) if existing_link_set else "无",
                context="\n\n---\n\n".join(page_contents),
            )
            ...
            async for chunk in llm.generate_text_stream(
                prompt=prompt,
                system_prompt=CONTEXTUAL_SUGGEST_SYSTEM_PROMPT,
                max_tokens=300,
            ):
                token, is_reasoning = self._stream_chunk_text(chunk)
                if is_reasoning:
                    full_thinking += token
            ...
            yield {
                ...
                "answer": answer,
                "action_taken": "queried",
                "related_pages": related_pages if answer else [],
                "thinking": full_thinking or None,
            }
        except Exception as e:
            logger.error(f"Contextual suggest stream error: {e}")
            # Contextual suggestions are opportunistic. Keep failures silent so
            # writing is never interrupted by advanced-assist errors.
            yield {"type": "done", "answer": "", "action_taken": "queried", "related_pages": []}
